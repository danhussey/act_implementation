...
- Initial observation media included a low-dim state panel; that low-dim panel was later removed from README media in favor of a camera-only rollout animation.
- Clarified camera semantics: the current vision runs use third-person `agentview`, not wrist/POV. That can still be occluded by the robot hand, so a stronger next vision variant should consider wrist view, multiple cameras, augmentation, or state estimation.
- Updated `observe-rollout`: `--camera` is the policy-input camera, `--view-camera` is the display camera, and matching image cameras produce a single panel instead of duplicating the same frame. Verified `--view-camera robot0_eye_in_hand --camera agentview` works for side-by-side inspection.
- Improved low-dim observation media from normalized feature bars to a spatial sketch: top-down end-effector/object positions, z ruler, distance readout, and a compact normalized-state strip. Regenerated the README GIFs after checking first-frame layout for clipped titles/overlaps.
- Removed the low-dim state schematic from README media for now. The low-dim cell is just the camera rollout animation; vision clips remain single `agentview` policy-input panels with cache-busting filenames.
